...
//		courService.addCourse(courseId, new Student(), new Professor(), list, board, roster);	
	...
	public List<Course> getCoursesByProfessor(@PathParam("professorId") String professorId){
		return courService.getCourseByProfessor(professorId);
	}
			
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public List<Course> getCoursesByTA(@QueryParam("taId") String taId){
		return courService.getCourseByTA(taId);
	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public Course updateCourse(@PathParam("courseId") String courseId, Course cour) {
		return courService.updateCourseInformation(courseId, cour);
	}
